...
- Relate the intro paragraph: "Scientists declared that 2015 was Earth’s hottest year on record..." to the *design* of the visualization.  In particular, based on the intro paragraph,
  - What key message/claim does NYTimes want readers to be able to explore? 
    NYTimes wants readers to explore the claim that 2015 was the Earth's hottest year on record.
  - How did this goal inform what information is displayed in the visualization?
  This goal contributed to the addition of multiple different features being displayed on the visualization. Their is a comparison for 2015 temperatures compared to average temperatures at that time and arrows that point out if record highs occurred on that day of the year.
...
geom_col() and geom_bar() are the only ones that seem like they could possibly create this visualization
...
ChatGpt prompt: "This is where I'm at now: "ggplot(weather) + geom_linerange(aes(x = dateInYear, ymin = RecordLow, ymax = RecordHigh), color = '#ECEBE3') + theme_classic() + geom_linerange(aes(x = dateInYear, ymin = NormalLow, ymax = NormalHigh), color = "#C8B8BA") + geom_linerange(aes(x = dateInYear, ymin = Low, ymax = High), color = "#A90248")" next step: Change the x-axis labels so that the month names display in the center of each month's slice of the plot. ::: {.callout-tip title="Month Names"} R has built-in variables called month.abb and month.name that contain abbreviated and full month names."
