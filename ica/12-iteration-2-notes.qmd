...
# 2. Empty storage vector 
col_classes <- vector("character", n_cols)

# 3. Loop through columns
for (i in seq_len(n_cols)) {
  # 4a. Extract column
  current_col <- mtcars[[i]]
  # 4b. Get class
  current_class <- class(current_col)
  # 4c. Store result
  col_classes[i] <- current_class
}
...
stats_list <- map(diamonds_num, summ_stats)
...
```{r}
n_cols <- ncol(diamonds_num)

stats_list_loop <- vector("list", n_cols)

for (i in seq_len(n_cols)) {
  
  current_col <- diamonds_num[[i]]
  
  stats_list_loop[[i]] <- summ_stats(current_col)
}

# Combine and add names
stats_df_loop <- bind_rows(stats_list_loop)
stats_df_loop$variable <- colnames(diamonds_num)

stats_df_loop
```


I prefer for_loops here just because I've got a lot more experience with them so it makes more sense to me but map is probably the more efficient way to write the code if you are equally good at both.
...
tibble(
  string = c("statistics", "regression", "modeling"),
  start  = c(1, 2, 3),
  end    = c(4, 5, 6)
) |>
  pmap_chr(str_sub)
