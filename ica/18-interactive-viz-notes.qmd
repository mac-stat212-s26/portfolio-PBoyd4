...
- ability to click on individual points on graph
- zoom in/out
- an "about" page
- ability to change area for graphs metrics
...
Yes, it allows you to explore trends closely or focus on certain areas one at a time. A bunch of static vizualizations all at once might be too much to handle.

    - What explorations/comparisons are you curious to explore that are not enabled well by the app?
Maybe something that compares cities across the country.
...
```{r reading}
#| eval: false
data_by_dist <- read_rds("neighborhood_diversity/data/diverse_data_by_dist.rds")
...
they access the current value of an input object
