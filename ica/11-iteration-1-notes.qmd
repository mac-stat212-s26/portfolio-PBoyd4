...
months_vector <- character(length(month.name))
for(i in seq_along(month.name)) {
  months_vector[i] <- str_c("Month ", i, ": ", month.name[i])
}
months_vector
...
lin_mod_results <- vector(mode = "list", length = length(cuts))
...
remove_outliers <- function(data, what_var, sd_thresh = 3) {
  data |>
    mutate(
      mean_val = mean({{ what_var }}, na.rm = TRUE),
      sd_val   = sd({{ what_var }}, na.rm = TRUE)
    ) |>
    filter(
      abs({{ what_var }} - mean_val) <= sd_thresh * sd_val
    ) |>
...
fit_model <- function(data, mod_formula, remove_outliers = FALSE) {
    ...
    model <- lm(mod_formula, data = data_clean)
...
model_list <- vector("list", nrow(df_arg_combos))

for (i in seq_len(nrow(df_arg_combos))) {
  
  current_formula <- df_arg_combos$mod_formula[[i]]
  current_outlier_flag <- df_arg_combos$remove_outliers[i]
  
  model_list[[i]] <- fit_model(
    data = diamonds,
    mod_formula = current_formula,
    remove_outliers = current_outlier_flag
  )
}
