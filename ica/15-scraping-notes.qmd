...
function to read over website text and scrape several pages article titles, publication dates, and abstract text and then create df with tibble. function with for loop that saves each page of info

- What arguments will they have? 
html_elements, html_text. the path to the titles, publication dates, and abstract text across several pages tibble to create dataset with new variables

- How will you use your functions? 

Consult with your peers and compare plans.

We want one final df with article titles, publication dates, and abstract text for news releases across several pages of results. To do this, we need an html_elements function that has the .description for each element we need and html_text to actually write this out.
...
```{r}
get_text <- function(page, css_selector_path) {
  page |> 
    html_elements(css_selector_path) |> 
    html_text()
}

get_news_page <- function(url) {
  page <- read_html(url)
  article_titles <- get_text(page, ".thumbnail-teaser__heading")
  abstract_text <- get_text(page, ".thumbnail-teaser__description")
  
  tibble(title = article_titles, date_abstract = abstract_text) 
}
  
scrape_x_pages_nih <- function(x) {
  base_url <- "https://www.nih.gov/news-events/news-releases"
  x <- x-1
  for (i in x) {
    page_url <- "https://www.nih.gov/news-events/news-releases" + "?page=" + "x"
    get_news_page(page_url)
    x <- x + 1
  }
}
