...
1. **Missing data mechanism** For each of the following situations, propose what missing data mechanism you think is most likely at play. 
  ...
  MCAR(assuming random equipment issues unrelated to the weather values)
...
ozone_miss <- airquality |> 
  mutate(miss_ozone = is.na(Ozone)) 
  
ozone_miss |> 
  ggplot(aes(x = miss_ozone, y = Temp)) +
  geom_boxplot()
  
ozone_miss |> 
  ggplot(aes(fill = miss_ozone, x = Month)) +
  geom_bar()

ozone_miss |> 
  ggplot(aes(x = miss_ozone, y = Wind)) +
  geom_boxplot()
  
lm(Ozone ~ Temp + Wind + Month, data = airquality)
