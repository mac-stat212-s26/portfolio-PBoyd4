...
phone <- function(x) {
  ...
  str_replace(digits, "(\\d{3})(\\d{3})(\\d{4})", "(\\1) \\2-\\3")
}
...
The first unnamed argument after the pipe is supposed to be the mapping, not the data.
...
temp_converter <- function(value, unit) {
  if (unit == "C") {
    return((value * 9/5) + 32)
  } else {
    return((value - 32) * 5/9)
...
domain_extraction <- function(email) {
  if (str_detect(email, "\\.[A-Za-z]+$")) {
    str_extract(email, "(?<=@)[^@\\s]+\\.[A-Za-z]+$")
  } else {
    return("Invalid email")
  }
}
domain_extraction("paxtonboyd@gmail.com")

domain_extraction("user@macalester.edu")

domain_extraction("not_an_email")
```
...
prop_count <- function(df, group_var) {
  df |>
    count(pick({{ group_var }}), name = "n") |>
    mutate(prop = n / sum(n))
}

prop_count(diamonds, cut)
prop_count(diamonds, c(cut, color))
...
scatter(diamonds, carat, price)

```

9. **extra bar plot function:**

```{r}
bar_plot <- function(df, x_var) {
  df |>
    ggplot(aes(x = {{ x_var }})) +
