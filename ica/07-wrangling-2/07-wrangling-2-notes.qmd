...
# Your code
str_subset(fruit, "aa|ee|ii|oo|uu")
...
# Your code
head(str_subset(words, "^y"),1)
head(str_subset(words, "^y", negate = TRUE),2)
...
pattern <- regex("\\b(mom|dad|family|husband|wife|marriage|money|work)\\b")

posts |> 
  mutate(n = str_count(question_only, pattern)) |> 
  arrange(desc(n)) |> 
  select(question_only, n) |> 
  slice_head(n = 10)

posts |> 
  mutate(has_match = str_detect(question_only, pattern)) |> 
  count(has_match)

matches <- str_subset(posts$question_only, regex("sex", ignore_case = TRUE))
    length(matches)
    head(matches, 10)
