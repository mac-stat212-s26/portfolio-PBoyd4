...
I would expect the overlay to be less accurate than data that was in NAD27 CRS since this CRS is designed specifically for North America.
...
designed to be perceived by viewers with common forms of color blindness

- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?

Use viridis_d with discrete data, c with continuous data, and b to bin continuous data before mapping. 
...
elevation <- elevatr::get_elev_raster(mn_counties |> st_crop(seven_countyarea) |>  st_cast("MULTIPOLYGON"), z = 9, clip = "bbox") 
raster::crs(elevation) <- sf::st_crs(mn_counties)
...
png("tc_map_zoom.png", width = 1200, height = 1200)
