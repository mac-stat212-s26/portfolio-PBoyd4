...
fit_mod_and_extract <- function(data, yvar, preds, pred_of_interest) {
  ...
  if (!is.character(preds) || length(preds) < 1) {
    stop("preds must be a character vector of predictor names")
  }
  
  if (!is.character(pred_of_interest) || length(pred_of_interest) != 1) {
    stop("pred_of_interest must be a length-1 character vector")
  }
  
  if (!(pred_of_interest %in% preds)) {
    stop("pred_of_interest must be one of the predictors in preds")
  }
  
  if (!(yvar %in% names(data))) {
    stop("yvar is not a column in data")
  }
  
  if (!all(preds %in% names(data))) {
    stop("One or more predictors in preds are not columns in data")
  }
